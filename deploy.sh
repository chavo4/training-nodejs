...
if [ ! -e ./chavo-key ]
then
    ssh-keygen -t rsa -b 4096 -f ./chavo-key -N ''
fi
...
$terraform apply -auto-approve -var app_name=${APP} -var environment=${ENVIRONMENT} -var ssh_key="$(cat ./chavo-key.pub)" ./terraform

sleep 10

echo "Connect to Wordpress using below IP"
echo http://$(terraform output public_ip_wp):8000

echo "Connect to Netdata using below IP"
echo http://$(terraform output public_ip_wp):19999
